...
const AcidSynth = () => {
    ...
    const [root, setRoot] = useState(0); // 0 = C, 1 = C#, etc.
    ...
    const baseCutoffRef = useRef(400);  // เก็บ cutoff frequency พื้นฐาน
    ...
    useEffect(() => {
        ...
        if (filterRef.current) {
            ...
            baseCutoffRef.current = cutoffFreq;  // อัพเดท base cutoff
        }
        ...
        for (let i = 0; i < patternLength; i++) {
            ...
            if (step && step.active) {
                ...
                Tone.Transport.schedule((time) => {
                    // Filter envelope
                    if (filterRef.current) {
                        const baseFreq = baseCutoffRef.current;  // ใช้ค่า cutoff ที่ตั้งไว้ ไม่ใช่ค่าปัจจุบัน
                    ...
                    // Update visual
                    setCurrentStep(i);
                }, i * 0.25);  // Time in beats (0.25 = 16th note)
            }
        }

        Tone.Transport.loop = true;
        Tone.Transport.loopEnd = patternLength * 0.25;  // Loop length in beats
    ...
    const generatePattern = useCallback(() => {
        ...
        // ถ้ากำลังเล่นอยู่ ให้เริ่มเล่น pattern ใหม่ทันที (ไม่ต้องหยุด)
    ...
    }, [scale, root]); // Re-quantize on scale or root change
    ...
    const exportMidi = useCallback(() => {
        ...
        const stepTicks = PPQ / 4; // 16th note = 120 ticks
        ...
        paramsRef.current.pattern.slice(0, patternLength).forEach((step, i) => {
            if (step.active) {
                ...
                const duration = stepTicks * (step.slide ? 1.25 : 0.8); // Slide overlaps slightly

                // Delta time for Note On
                const deltaOn = currentTime - lastEventTime;
                trackData.push(...encodeVLQ(deltaOn));
                trackData.push(0x90, midiNote, velocity); // Note On
        ...
        const headerChunk = [
            ...
            0x00, 0x01,             // 1 Track
            (PPQ >> 8) & 0xFF, PPQ & 0xFF // Resolution
        ];
    ...
    const gridNotes = useMemo(() => {
        ...
        return notes; // Sorted high to low if using reverse in render
    }, [scale]);
        ...
        <div className="min-h-screen bg-[#050505] text-zinc-400 p-2 sm:p-4 lg:p-8 font-mono select-none overflow-x-hidden">
            <div className="max-w-[1400px] mx-auto space-y-6">
                ...
                <div className="border border-zinc-800 bg-zinc-950 overflow-hidden shadow-2xl rounded-sm sm:rounded-md relative">
                    ...
                    <div className="overflow-x-auto overflow-y-hidden scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent">
                ...
                <footer className="flex justify-between items-center text-[8px] text-zinc-800 font-bold uppercase tracking-[0.2em] pt-4">
                    ...
                    <Link href="/" className="hover:text-zinc-600">EXIT SEQUENCE_</Link>
                </footer>
            </div>
