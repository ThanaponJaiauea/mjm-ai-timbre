...
export default function ChordRecommend({
  ...
  const playChords = async () => {
    ...
    setTimeout(
      ...
      recommendedChords.length * interval * 1000,
    );
  };
    ...
    try {
      ...
      const chordArray =
        typeof chords === "string"
          ? chords.split(",").map((c) => c.trim())
          : chords;
      const romanArray =
        typeof roman_numerals === "string"
          ? roman_numerals.split(",").map((r) => r.trim())
          : roman_numerals;
  ...
  const handleGenerate = async (payload) => {
    ...
    if (!data.chords || data.chords.length === 0) return;

    setLoading(true);
    try {
      const response = await generate_settings(data);
      setArp(response.data);
      onArpGenerated?.(response.data);
    } catch (error) {
        ...
        <div className="bg-[#1e1e1e] p-6 rounded-xl border-l-4 border-gray-600 animate-pulse">
          <div className="flex items-center gap-2 mb-4 text-gray-500">
            ...
            <span className="text-sm font-mono">
              AI กำลังเลือกคอร์ดที่เหมาะสมที่สุด...
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {!isAudioReady ? (
                ...
                <Play
                  size={16}
                  fill={activeChordIndex !== null ? "transparent" : "white"}
                />
              )}
              {!isAudioReady
                ? "LOADING AUDIO..."
